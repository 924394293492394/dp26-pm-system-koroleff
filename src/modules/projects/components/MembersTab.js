...
const MembersTab = ({ projectId, currentUserRole, currentUserId }) => {
  ...
  const { members, meta, loading, saving, fetch, add, updateRole, remove, leave } =
  ...
  const handleRoleChange = async (userId, newRole) => {
    ...
    if (result === true) message.success("Роль изменена");
    else message.error(result);
  };

  const handleRemove = async (userId, login) => {
    const result = await remove(userId);
    if (result === true) message.success(`${login} удалён из проекта`);
    else message.error(result);
  };
  ...
  const columns = [
          ...
          <div>
            <div>
              ...
              {record.userId === currentUserId && (
                <Tag color="blue" style={{ marginLeft: 6, fontSize: 11 }}>Вы</Tag>
              )}
            </div>
    ...
    },
    {
      ...
      width: 100,
      render: (_, record) => {
        if (!canManageMember(record.role, record.userId)) return null;
        return (
          <Popconfirm
